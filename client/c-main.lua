...
if Config.Framework == "QBCore" then
    ...
    RegisterCommand(Config.VehicleRob.command, function(args)
        ...
        if IsPedInAnyVehicle(ped, false) then
            ...
            QBCore.Functions.Notify(LC['Correcta_Alerta'])
            TriggerServerEvent("Opto_dispatch:Server:SendVehRob", 'police', coords, model, placa, color, id)
        else
...
AddEventHandler("Opto_dispatch:Client:SendVehRob", function(coords, model, placa, color, id)
    numcall = numcall + 1
    allcalls = allcalls + 1
    ...
    if callnum == totalcalls then
        SendNUIMessage({
            type = 'alert',
            title = LC['Alerta_Titulo'],
            content = LC['Veh_Rob_01'] .. model .. " color " .. color .. " - placa: " .. placa .. LC['Veh_Rob_02'] .. streetname,
            numcall = numcall,
            allcalls = allcalls,
            id = id
        });
        ...
        table.insert(calls,
            { text = LC['Veh_Rob_01'] .. model .. " color " .. color .. " - placa: " .. placa .. LC['Veh_Rob_02'] .. streetname, coords = coords,
                title = LC['Alerta_Titulo'] })
